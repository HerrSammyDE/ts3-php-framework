...
class TeamSpeak3_Node_Host extends TeamSpeak3_Node_Abstract
{
  ...
  public function serverCreate(array $properties = array())
  {
    ...
    if (!isset($detail["sid"]))
        $detail = end($detail);
    $server = new TeamSpeak3_Node_Server($this, array("virtualserver_id" => intval($detail["sid"] ?? null)));

    TeamSpeak3_Helper_Signal::getInstance()->emit("notifyServercreated", $this, $detail["sid"] ?? null);
    TeamSpeak3_Helper_Signal::getInstance()->emit("notifyTokencreated", $server, $detail["token"] ?? null);
